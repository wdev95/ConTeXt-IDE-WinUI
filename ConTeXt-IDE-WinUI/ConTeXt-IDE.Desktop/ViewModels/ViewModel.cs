...
using RelayCommand = ConTeXt_IDE.Shared.Helpers.RelayCommand;
...
namespace ConTeXt_IDE.ViewModels
{
 public class ViewModel : Helpers.Bindable
 {
  ...
  public MarkdownConfig MDC { get => Get(new MarkdownConfig()); set => Set(value); }
  ...
  public string CurrentProjectPathKey => NormalizeProjectPath(CurrentProject?.Path);


  private string rootFilePath;

  public RelayCommand UpdateFontDatabaseCommand { get; set; }

  private async void UpdateFontDatabase()
  {
	try
	{
	 ...
	 try
	 {
	  ...
	  string logtext = "Updating font database";

	  Log(logtext);

	  await Task.Run(async () =>
	  {
		await UpdateOSFontDirTask();
		await UpdateFontDatabaseTask();
	  });
	...
	{
	 Log("Exception at font update: " + f.Message);
	}
  }

  public async Task<bool> UpdateFontDatabaseTask()
  {
	Process p = new();
	ProcessStartInfo info = new()
	{
	 ...
	 WorkingDirectory = Default.ContextDistributionPath
	};

	p.StartInfo = info;
	info.FileName = Default.ContextDistributionPath + @"\tex" + getversion() + @"\bin\mtxrun.exe";
	info.Arguments = "--generate";

	p.OutputDataReceived += (a, b) =>
	{
	 if (Default.ShowCompilerOutput)
	...
	info.FileName = Default.ContextDistributionPath + @"\tex" + getversion() + @"\bin\mtxrun.exe";
	info.Arguments = "--script font --reload";
  ...
  public async Task<bool> UpdateOSFontDirTask()
  {
	Process p = new();
	...
	p.StartInfo = info;
	info.FileName = "cmd";
	info.Arguments = "/C set OSFONTDIR=c:/windows/fonts";
  ...
  {
	if (Directory.Exists(Default.ContextDistributionPath + @"\tex\texmf-win64"))
	{
	 return @"\texmf-win64";
	}
	else if (Directory.Exists(Default.ContextDistributionPath + @"\tex\texmf-mswin"))
	{
	 return @"\texmf-mswin";
	}
	else if (Directory.Exists(Default.ContextDistributionPath + @"\tex\texmf-windows-arm64"))
	{
  ...
  public ViewModel()
  {
	UpdateFontDatabaseCommand = new RelayCommand(UpdateFontDatabase);
	...
	try
	{
	 ...
	 Default.ProjectList.CollectionChanged += (s, e) =>
		{
		 OnPropertyChanged(nameof(CurrentProjectPathKey));
		};
  ...
  //public enum Symbol { get => Get(FluentIcon); set => Set(value); }
  ...
  public FileItem CurrentFileItem
  {
	...
	set
	{
	 ...
	 {
	  MDC = new MarkdownConfig() { BaseUrl = CurrentRootItem.FileFolder + "/" };
  ...
  {
	if (text == null)
	 text = CurrentFileItem.Editor.Lines.ToList();
  ...
  {
	if (text == null)
	 text = String.Join("\r\n", CurrentFileItem.Editor.Lines.Select(x => x.LineText));
	...
	set
	{
	 if (value != null)
	 {
	  ...
	  if (value.Folder != null)
	  {
		...
		IsTeXError = false;
		App.MainWindow?.AW?.Title = value.Name;
		Log("Project opened: " + value.Name);
	  }
	 }
	 else
	 {
	 ...
	 Set(value);
	 OnPropertyChanged(nameof(CurrentProjectPathKey));
	 RefreshOpenedProjectMarkers();
	 App.UpdateCurrentInstanceProjectKey(value?.Path);
	}
  }
  ...
  public Thickness InfobarMargin { get => Get(new Thickness(RibbonMarginValue, 0, RibbonMarginValue, 0)); set => Set(value); }

  public int RibbonMarginValue
  {
	get => Get(Default.RibbonMarginValue);
	set
	{
	 Set(value);
	 CornerRadius_Ribbon = new(value * 2);
	 Margin_Ribbon = new(value, 0, value, value);
	 CornerRadius = new(value);
	 LargeCornerRadius = new CornerRadius(2 * value);
	 CornerRadiusRight = new CornerRadius(0, value, value, 0);
	 CornerRadiusLeft = new CornerRadius(value, 0, 0, value);
	}
  }
  public Thickness Margin_Ribbon { get => Get(new Thickness(RibbonMarginValue, 0, RibbonMarginValue, RibbonMarginValue)); set { Set(value); InfobarMargin = value; } }
  public CornerRadius CornerRadius_Ribbon { get => Get(new CornerRadius(RibbonMarginValue * 2)); set => Set(value); }
  public CornerRadius CornerRadius { get => Get(new CornerRadius(RibbonMarginValue)); set => Set(value); }
  public CornerRadius LargeCornerRadius { get => Get(new CornerRadius(2 * RibbonMarginValue)); set => Set(value); }
  public CornerRadius CornerRadiusRight { get => Get(new CornerRadius(0, RibbonMarginValue, RibbonMarginValue, 0)); set => Set(value); }
  public CornerRadius CornerRadiusLeft { get => Get(new CornerRadius(RibbonMarginValue, 0, 0, RibbonMarginValue)); set => Set(value); }
  ...
  public CodeWriter CurrentCodeWriter { get => Get<CodeWriter>(null); set { Set(value); } }
  ...
  {
	get => Get(IsInternalViewerActive | IsMarkdownViewerActive); set
	{
	 Set(value);
	}
  }

  public bool IsInternalViewerActive { get => Get(false); set { Set(value); if (value) IsViewerActive = true; } }
		...
		_queue.TryEnqueue(async () =>
				  {
					...
					if (item != null)
					{
					 ...
					 {
					  AddFileItemAlphabetically(containingfolder.Children, fi);
  ...
  public static void AddFileItemAlphabetically(ObservableCollection<FileItem> Children, FileItem fi)
  {
  ...
  public FileItem FindFileInCurrentProject(string filePath)
  {
	if (string.IsNullOrWhiteSpace(filePath))
	 return null;

	if (CurrentProject?.Directory?.Count > 0)
	{
	 return CurrentProject.GetFileItemByPath(CurrentProject.Directory[0], filePath);
	}

	return null;
  }

  public void OpenActivatedStorageFile(StorageFile storageFile)
  {
	if (storageFile == null)
	 return;

	var projectFile = FindFileInCurrentProject(storageFile.Path);
	if (projectFile != null)
	{
	...
	var externalFile = new FileItem(storageFile)
	{
	 IsExternal = true
	};
	OpenFile(externalFile);
  }
  ...
  public async Task<bool> LoadProject(Project proj)
  {
	try
	{
	 if (Default.MultiInstance && await App.TryRedirectProjectLaunchToOpenedInstanceAsync(proj?.Path))
	 {
	  return false;
	 }
  ...
  public void RefreshOpenedProjectMarkers()
  {
	string NormalizePathSafe(string path)
	{
	 if (string.IsNullOrWhiteSpace(path))
	  return string.Empty;
	 try
	...
	string currentProjectPath = NormalizePathSafe(CurrentProject?.Path);
	string currentProjectName = CurrentProject?.Name ?? string.Empty;

	foreach (var project in Default.ProjectList)
	{
	 if (project == null)
	  continue;

	 bool isCurrentProject = project == CurrentProject
		  || (!string.IsNullOrWhiteSpace(currentProjectPath) && string.Equals(NormalizePathSafe(project.Path), currentProjectPath, StringComparison.OrdinalIgnoreCase))
		  || (string.IsNullOrWhiteSpace(currentProjectPath) && !string.IsNullOrWhiteSpace(currentProjectName) && string.Equals(project.Name, currentProjectName, StringComparison.OrdinalIgnoreCase));

  project.IsCurrentInThisWindow = isCurrentProject;
  project.IsOpenedInAnyInstance = isCurrentProject || (Default.MultiInstance && App.IsProjectOpenedInAnotherInstance(project.Path));
	 project.CanOpenInNewInstance = Default.MultiInstance && !project.IsOpenedInAnyInstance;
	}
  }
  ...
  public async void PopulateJumpList()
  {
	try
	{
	 ...
	 {
	  JumpListItem jli = JumpListItem.CreateWithArguments(item.Path, item.Name);
	  jli.GroupName = "ConTeXt Projects";
	  jli.Description = item.Path;
	  jli.Logo = new("ms-appx:///Assets/Logo_icon_2025_File.png");
	  jl.Items.Add(jli);
	 }
  ...
  if (!hasPendingFileActivations && !SuppressLastActiveProjectOnStartup && Default.StartWithLastActiveProject && !string.IsNullOrWhiteSpace(Default.LastActiveProject) && string.IsNullOrWhiteSpace(LaunchArguments))
	 ...
	 else if (!hasPendingFileActivations && !string.IsNullOrWhiteSpace(LaunchArguments))
	 {
	  IsSaving = true;
	  var list = Default.ProjectList.Where(x => x.Path == LaunchArguments);
	  if (list != null && list.Count() == 1)
	  {
		var project = Default.ProjectList.FirstOrDefault(x => x.Path == LaunchArguments);
		CurrentProject = project;
	  }
	 }
	...
	if (filetosave != null)
	 if (filetosave.File != null)
	 {
	  try
	  {
		...
		await FileIO.WriteBufferAsync(filetosave.File as StorageFile, buffer);
  ...
  public async Task SaveAll()
  {
	try
	{
	 ...
	 foreach (var item in FileItems)
	 {
	  if (item?.File is not StorageFile)
		continue;
